...
class ScriptEvaluator:
    ...
    def main(self):
        ...
        epochs = self.model_file.split('_')[1]

        # Store accuracy in a file
        accuracy_file = os.path.join(ScriptEvaluator.RESULTS_DIR, f'{model_size}_{epochs}Epochs_accuracy.txt')
        with open(accuracy_file, 'w') as f:
            f.write(f"Accuracy: {accuracy * 100:.2f}%\n")

        # Store results in a CSV file
        results_file = os.path.join(ScriptEvaluator.RESULTS_DIR, f'{model_size}_{epochs}Epochs_results.csv')
