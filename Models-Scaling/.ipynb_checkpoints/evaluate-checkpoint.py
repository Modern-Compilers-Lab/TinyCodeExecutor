...
class ScriptEvaluator:
    ...
    def load_model(self, model_file):
        # Extract n_embd from the model filename
        n_embd = int(model_file.split('_')[1])
        
        # Initialize the model with the extracted n_embd
        model = GPT(n_embd=n_embd)
        print(f"Compiling model with n_embd = {n_embd} from {model_file}...")
    ...
    def main(self):
        ...
        accuracy_file = os.path.join(ScriptEvaluator.RESULTS_DIR, f'{model_size}_accuracy.txt')
        with open(accuracy_file, 'w') as f:
            f.write(f"Accuracy: {accuracy * 100:.2f}%\n")

        # Store results in a CSV file
        results_file = os.path.join(ScriptEvaluator.RESULTS_DIR, f'{model_size}_results.csv')
